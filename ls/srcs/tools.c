...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/05/14 02:01:05 by pvalleci          #+#    #+#             */
/*   Updated: 2019/05/14 02:01:13 by pvalleci         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char			**ft_buf_tab(char **tab, int nb_column, int	len_line, int len_buf)	
{
	...
	while (i < nb_line)
	{
		if (!(ioctl_tab[i] = ft_strnew(len_line)))
			return (NULL);
		i++;
	}
	ioctl_tab[i] = NULL;
	i = 0;
	ft_input_tab_ioctl(ioctl_tab , tab, len_buf);
	ft_free_tab(tab);
	return (ioctl_tab);
}
