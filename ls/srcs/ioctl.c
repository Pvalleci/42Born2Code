...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/05/14 02:00:27 by pvalleci          #+#    #+#             */
/*   Updated: 2019/05/14 02:00:30 by pvalleci         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_display_ioctl(char **file_tab, char *option)
{
	...
	if (nb_column == 0)
		nb_column++;
	// printf("nb_column : %d\n", nb_column);
	while ((nb_column * ((ft_len_tab(tmp_tab) / nb_column)))
	> (ft_len_tab(tmp_tab) + ((ft_len_tab(tmp_tab) / nb_column) + 1)))
		nb_column--;
